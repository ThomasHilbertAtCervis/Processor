...
The core executable palette stays small. `python` nodes still support
inline `if`/`for`/`foreach` for one-off expressions; dedicated control-flow
nodes exist for cases where the body of a branch or loop is itself a
non-trivial subgraph (sub-module calls, db nodes, multi-step pipelines).
Composition with other modules is expressed via `submodule` nodes.
...
|  ◆     | Branch          | One input `value`; static parameter `condition` (Python expression, sees the payload as `value`). Fires on output `true` or `false` depending on the expression's truthiness. Both outputs carry the original value unchanged and share its data type. |
|  ◇     | Join            | Inputs `a`, `b` (more can be added); each arrival is forwarded immediately to the single output `value`. Used to merge alternative branches back into one wire. |
|  ↻     | Counted Loop    | Inputs `from`, `to` (ints); once both arrive, fires `index` for every value in `range(from, to)`, then fires `done` with the iteration count. Iterations run synchronously — each downstream chain completes before the next iteration starts. |
|  ⇄     | For Each        | Input `collection` (list, tuple, or dict). Fires `key` + `item` per element (`key` is the index for lists / the mapping key for dicts), then `done` with the original collection. |
...
| F-024 | Owner direction, May 2026             | *"We need more node types for branching on a condition, joining branches back together, counted loops (from x to y, where x and y must be dynamic) and foreach-loops (e.g. for each item of an array or dictionary)."* | ✅ — four new node kinds added to the wire graph: `branch` (one `value` input + static `condition` Python expression; routes the value unchanged to `true` or `false`, both outputs share the input's data type), `join` (forwards every input arrival to `value`; OR-merge for reconverging branches), `counted_loop` (inputs `from`/`to`; fires `index` per iteration of `range(from, to)`, then `done`), `foreach` (input `collection`, list/tuple/dict; fires `key` + `item` per element, then `done`). All four are pure data-flow nodes — they live in the wire graph alongside `python`, `submodule` and `db_*`. Activators registered in `Simulator._ACTIVATORS`; tests in `tests/test_simulator.py` cover happy paths, expression evaluation, empty ranges, dict iteration, and bad-input errors. Frontend palette and properties panel pick the new kinds up automatically through `/api/node-kinds`. *Known limitation:* because the model has no shared variables, accumulating an array across iterations still requires either an inline `python` node with a `for` loop or writing rows via `db_create`; first-class "fold/reduce" semantics remain on the backlog.|
| F-024a| Owner clarification, May 2026         | *"The branch node should have a constant parameter for the condition, like the data type and signal_name on inputs. The condition should be a python expression that can access the value that's received on the 'value' input. The 'true' and 'false' outputs should have the same data type as the 'value' input, and should output the input value unchanged."* | ✅ — `branch` redesigned: the `condition` input port is gone, replaced by a static `condition` field on `node.data` that holds a Python expression evaluated through `SafeScriptInterpreter.evaluate_expression` with the incoming payload bound to `value`. The properties panel offers a `Value data type` dropdown that updates the `value`, `true`, and `false` ports together so all three always agree. |
| F-025 | Owner direction, May 2026             | *"`any` shouldn't be an option for users to select as a type. I want strong typing. Mismatched types should be detected right away and shown in red, so the user can fix them."* | ✅ — `any` removed from the primitives catalog (`PRIMITIVE_TYPE_IDS`) so it no longer appears in any UI dropdown (data-type editor, port-type pickers, branch value-type, module signal-type). Newly-dropped nodes seed `string` instead of `any` for their default port `type_ref`. The diagram canvas tags every edge whose source `type_ref` differs from its target `type_ref` with `className="type-mismatch"`; CSS (`.react-flow__edge.type-mismatch`) renders it in red so the user spots and fixes the mismatch immediately. Legacy stored modules that still carry `type_ref="any"` keep loading (backend tolerates the value), but every edge touching such a port mismatches a typed port and is highlighted until the user picks a real type. |
| F-025a| Owner clarification, May 2026         | *"It shouldn't be necessary to specify the data type of an edge/wire. It should automatically have the type of the output it is connected to."* | ✅ — the Connection properties panel no longer offers a `Signal Type` dropdown; it now shows the source output's `type_ref` as a read-only `Data type` field. Wires are pure conductors of whatever the upstream port produces. |
| F-025b| Owner clarification, May 2026         | *"Some nodes have dynamic input types, 'Branch' for example. They should automatically get their input type assigned by the wire connected to it. ... 'Module Input' is allowed to be of type 'any'."* | ✅ — `onConnect` propagates the source output's `type_ref` onto the target input port for `module_output`, `python`, `branch`, `join`, `counted_loop`, `foreach`, `db_read`, `db_create`. Special cases: `branch` mirrors the inherited type onto its `value`/`true`/`false` ports and `data.value_type`; `join` mirrors it onto every input port plus the `value` output (so all join inputs are forced to agree). `module_input` and `submodule` are exempt — Module Input's type is user-declared (and may pick `any`, which is the one place the dropdown still offers it), and Submodule input types follow the linked module's signature. |
